                         ...
                         int flag)
{
 ...
 if(flag & 1) {
   ...
   xorriso->indev_md5_was_on= (xorriso->do_md5 & 1);
 }
...
"This might be false MD5 alarm if an add-on session was written by growisofs.",
 ...
 if(!(flag&32)) {
   ...
   if(xorriso->loaded_volid_pvd[0] != 0 &&
      (state == BURN_DISC_APPENDABLE || state == BURN_DISC_FULL)) {
       sprintf(xorriso->info_text,"Volume id    : '%s'\n",
               xorriso->loaded_volid_pvd);
       Xorriso_info(xorriso, 0);
   }
   if(strcmp(xorriso->loaded_volid_pvd, xorriso->volid_pvd) != 0 &&
...
no_tracks_found:;
   ...
   /* Possibly check relocated superblock of overwritable medium */
   if(num_sessions > 0) {
     iso_burn_tracks= isoburn_toc_session_get_tracks(isoburn_sessions[0],
                                                     &num_tracks);
     if(num_tracks > 0) {
       isoburn_toc_track_get_entry(iso_burn_tracks[0], &isoburn_entry);
       if(!(isoburn_entry.extensions_valid & 1)) /* should not happen */
         track_lba= -1;
       else
         track_lba= isoburn_entry.start_lba;
       if(track_lba == 32) {
         /* Looks like an ISO for overwritable medium: Check LBA range 0 - 31
         */
         ret= Xorriso_check_interval(xorriso, *spotlist, job,
                                  (off_t) 0, (off_t) 32, read_chunk, (off_t) 0,
                                  (4 * (xorriso->do_md5 & 1)));
         if(ret <= 0)
           goto ex;
       }
                           ...
                           int flag)
{
 ...
 static char *ignored_names[]= {"size", "eltorito", "tree_loaded",
                                ...
                                "rrip_1_10_px_ino",
                                ""};
 static char *relaxed_pvd_modes[]= {"off", "vol", "nonvol", "on"};
 ...
 } else if(strcmp(name, "relaxed_vol_atts") == 0) {
   if(as_m) {
     if(val_num >= 0.0 && val_num <= 3.0)
       sprintf(xorriso->result_line, "-relaxed_pvd_atts %s",
                                     relaxed_pvd_modes[(int) val_num]);
   } else {
     if(val_num >= 0.0 && val_num <= 3.0)
       sprintf(xorriso->result_line, "-compliance relaxed_pvd_atts=%s",
                                     relaxed_pvd_modes[(int) val_num]);
   }
 } else {
