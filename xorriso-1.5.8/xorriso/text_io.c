...
int Xorriso_write_session_log(struct XorrisO *xorriso, int flag)
{
 ...
 wpt= sfe;
 for(rpt= xorriso->volid_pvd; *rpt!=0; rpt++) {
   if(*rpt=='\n') {
     *(wpt++)= '\\';
     *(wpt++)= 'n';
   } else
...
/* @param flag bit0=test over length of line rather than length of filter 
*/
                          ...
                          int flag)
{
 if(filter!=NULL)
   if(filter[0]=='-') {
     if(flag & 1) {
       if(strncmp(filter, line, strlen(line)) != 0)
         return(0);
     } else {
...
*/
{
 ...
 int show_indev= 1, show_outdev= 1, show_dev= 0, is_volid_filter;
 ...
 is_volid_filter=
                 (Xorriso_status_filter(xorriso, filter, "-volid_for", 0) > 0);
 if(strcmp(xorriso->volid_pvd, xorriso->volid_joliet) == 0 &&
    strcmp(xorriso->volid_pvd, xorriso->volid_1999) == 0 &&
    strcmp(xorriso->volid_pvd, xorriso->volid_hfsplus) == 0 &&
    (filter == NULL ||
     Xorriso_status_filter(xorriso, filter, "-volid_for", 1) <= 0)) {
   sprintf(line, "-volid %s\n", Text_shellsafe(xorriso->volid_pvd, sfe, 0));
   if(!(is_default && no_defaults))
     Xorriso_status_result(xorriso, filter, fp, flag & 2);
 } else {
   sprintf(line, "-volid_for ecma119 %s\n",
          Text_shellsafe(xorriso->volid_pvd, sfe, 0));
   Xorriso_status_result(xorriso, filter, fp, flag & 2);
   sprintf(line, "-volid_for joliet %s\n",
          Text_shellsafe(xorriso->volid_joliet, sfe, 0));
   Xorriso_status_result(xorriso, filter, fp, flag & 2);
   sprintf(line, "-volid_for iso_9660_1999 %s\n",
          Text_shellsafe(xorriso->volid_1999, sfe, 0));
   Xorriso_status_result(xorriso, filter, fp, flag & 2);
   sprintf(line, "-volid_for hfsplus %s\n",
          Text_shellsafe(xorriso->volid_hfsplus, sfe, 0));
   Xorriso_status_result(xorriso, filter, fp, flag & 2);
 }
 if(is_default && xorriso->loaded_volid_pvd[0] &&
    strcmp(xorriso->loaded_volid_pvd, xorriso->volid_pvd) != 0 &&
    (is_volid_filter || !no_defaults)) {
   sprintf(line,"# loaded image effective -volid_for ecma119 %s\n",
           Text_shellsafe(xorriso->loaded_volid_pvd, sfe, 0));
   Xorriso_status_result(xorriso, is_volid_filter ? "" : filter, fp, flag & 2);
 }
 if(is_default && xorriso->loaded_volid_joliet[0] &&
    strcmp(xorriso->loaded_volid_joliet, xorriso->volid_joliet) != 0 &&
    (is_volid_filter || !no_defaults)) {
   sprintf(line,"# loaded image effective -volid_for joliet %s\n",
           Text_shellsafe(xorriso->loaded_volid_joliet, sfe, 0));
   Xorriso_status_result(xorriso, is_volid_filter ? "" : filter, fp, flag & 2);
 }
 if(is_default && xorriso->loaded_volid_1999[0] &&
    strcmp(xorriso->loaded_volid_1999, xorriso->volid_1999) != 0 &&
    (is_volid_filter || !no_defaults)) {
   sprintf(line,"# loaded image effective -volid_for iso_9660_1999 %s\n",
           Text_shellsafe(xorriso->loaded_volid_1999, sfe, 0));
   Xorriso_status_result(xorriso, is_volid_filter ? "" : filter, fp, flag & 2);
 }
 ...
 is_default= (xorriso->perform_opc == 0);
 sprintf(line, "-perform_opc %s\n", xorriso->perform_opc == 1 ?
                                    "as_growisofs" :
                                    xorriso->perform_opc == 2 ? "on" : "off");
 if(!(is_default && no_defaults))
   Xorriso_status_result(xorriso,filter,fp,flag&2);
                               ...
                               int flag)
{
 ...
 if(warn_of_no_md5) {
   sprintf(xorriso->info_text,
           "Loading of MD5 was disabled when the session was loaded");
   Xorriso_msgs_submit(xorriso, 0, xorriso->info_text, 0, "WARNING", 0);
   sprintf(xorriso->info_text,
     "-md5 \"on\" before -dev or -indev enables detection and loading of MD5");
...
"This may be changed at compile time by ./configure option --enable-external-filters-setuid");
 ...
 if(cmd_pipe_adr[0] && reply_pipe_adr[0]) {
   ...
   for(i= 0; i < 2; i++) {
     ...
     if(ret == -1) {
       ret= mkfifo(adrpt, S_IRWXU | S_IRWXG | S_IRWXO | S_IRWXO);
       if(ret == -1) {
          sprintf(xorriso->info_text,
