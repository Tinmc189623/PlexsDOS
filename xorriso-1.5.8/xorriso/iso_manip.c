                         ...
                         IsoNode *node, int depth, int flag)
{
 ...
 } else if(action == 66) { /* show_time */
   ret= Xorriso_get_node_timestring(xorriso, node, target, time_string, 0);
   if(ret > 0) {
     if(strchr(target, 's') != NULL) {
       sprintf(xorriso->result_line, "%-18s ", time_string);
     } else {
       /* Format length suffices for year -0001 and 10000 in local time */
       sprintf(xorriso->result_line, "%-21s ", time_string);
     }
     Xorriso_esc_filepath(xorriso, show_path, xorriso->result_line, 1);
     strcat(xorriso->result_line, "\n");
     Xorriso_result(xorriso, 0);
