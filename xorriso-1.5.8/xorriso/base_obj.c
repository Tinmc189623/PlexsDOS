 ...
 *xorriso= m= TSOB_FELD(struct XorrisO,1);
 ...
 m->tree_loaded= 0;
 m->rr_loaded= 0;
 strcpy(m->volid_pvd, "ISOIMAGE");
 strcpy(m->volid_joliet, m->volid_pvd);
 strcpy(m->volid_1999, m->volid_pvd);
 strcpy(m->volid_hfsplus, m->volid_pvd);
 m->volid_default= 1;
 m->loaded_volid_pvd[0]= 0;
 m->loaded_volid_joliet[0]= 0;
 m->loaded_volid_1999[0]= 0;
 m->assert_volid[0]= 0;
