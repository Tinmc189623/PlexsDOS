...
/* xorriso - creates, loads, manipulates and burns ISO 9660 filesystem images.
   ...
   Copyright 2007-2026 Thomas Schmitt, <scdbackup@gmx.net>
 ...
 *burn_options= burn_write_opts_new(drive);
 ...
 burn_write_opts_set_perform_opc_v2(*burn_options, xorriso->perform_opc);
...
int Xorriso_format_media(struct XorrisO *xorriso, off_t in_size, int flag)
{
 ...
 if(flag&128) { /* by_index */
   ...
   if(flag & 32) {
     /* (libburn would refuse too, if not bit5 of mode_flag would get cleared
         below. So bail out here and be safe.) */
     sprintf(xorriso->info_text,
         "Detected BD-R. Thus omitting format run with mode 'without_spare'.");
     Xorriso_msgs_submit(xorriso, 0, xorriso->info_text, 0, "NOTE", 0);
     return(2);
   }
   if(!(flag & 2))
                                    ...
                                    int flag)
{
 ...
 continue;
   if((l == 6 && strncmp(cpt, "strict", l) == 0) ||
     ...
     xorriso->allow_dir_id_ext_dflt= 1;
     ...
     xorriso->iso_level= 3;
     xorriso->iso_level_is_default= 1;
     ...
     xorriso->max_ce_entries= 31;
     xorriso->max_ce_entries_flag= 2;
     ...
     xorriso->relax_compliance&= ~isoburn_igopt_rrip_tf_long;
   } else if((l == 13 && strncmp(cpt, "rrip_tf_year0", 13) == 0)) {
     xorriso->relax_compliance&= ~isoburn_igopt_rrip_tf_y1900;
   } else if((l == 13 && strncmp(cpt, "rrip_tf_y1900", 13) == 0)) {
     xorriso->relax_compliance|= isoburn_igopt_rrip_tf_y1900;
   } else if(l >= 17 && strncmp(cpt, "relaxed_pvd_atts=", 17) == 0) {
     if(strncmp(cpt + 17, "off", 3) == 0) {
       xorriso->relax_compliance&= ~(isoburn_igopt_relaxed_vol_atts |
                                     isoburn_igopt_relaxed_nonvol_atts);
     } else if(strncmp(cpt + 17, "vol", 3) == 0) {
       xorriso->relax_compliance&= ~isoburn_igopt_relaxed_nonvol_atts;
       xorriso->relax_compliance|= isoburn_igopt_relaxed_vol_atts;
     } else if(strncmp(cpt + 17, "nonvol" , 6) == 0) {
       xorriso->relax_compliance&= ~isoburn_igopt_relaxed_vol_atts;
       xorriso->relax_compliance|= isoburn_igopt_relaxed_nonvol_atts;
     } else if(strncmp(cpt + 17, "on", 2) == 0) {
       xorriso->relax_compliance|= (isoburn_igopt_relaxed_vol_atts |
                                    isoburn_igopt_relaxed_nonvol_atts);
     } else {
       sprintf(xorriso->info_text,
               "-compliance: unrecognized mode with relaxed_pvd_atts=");
       Xorriso_msgs_submit(xorriso, 0, xorriso->info_text, 0, "FAILURE", 0);
     }
   } else {
     if(l<SfileadrL)
...
int Xorriso_get_relax_text(struct XorrisO *xorriso, char mode[1024],
                           int flag)
{
 int r, drop, relax_att;
 static char *relax_att_name[4]= {"off", "vol", "nonvol", "on"};
 ...
 relax_att= (!!(r & isoburn_igopt_relaxed_vol_atts)) +
            2 * !!(r & isoburn_igopt_relaxed_nonvol_atts);
 strcat(mode, ":relaxed_pvd_atts=");
 strcat(mode, relax_att_name[relax_att]);
 if(r & isoburn_igopt_joliet_utf16)
   strcat(mode, ":joliet_utf16");
 ...
 if(r & isoburn_igopt_rrip_tf_y1900)
   strcat(mode, ":rrip_tf_y1900");
...
/* @param field_head Example: " pos="
*/
int Xorriso__set_iso_check_tag_off_t(char *tag_data, char *field_head,
                                     off_t value, int *field_end, int flag)
{
 char *cpt, *npt, *ept, *spt, num_text[81];
 int head_len, num_len, diff;

 head_len= strlen(field_head);
 if(head_len < 2)
   return(0);
 cpt= strstr(tag_data, field_head);
 if(cpt == NULL)
   return(0);
 npt= strchr(cpt + head_len, ' ');
 if(npt == NULL)
   npt= strchr(cpt, '\n');
 if(npt == NULL)
   return(0);
 ...
 sprintf(num_text, "%.f", (double) value);
 num_len= strlen(num_text);

 diff= num_len - ((npt - cpt) - head_len);
 if(diff > 0) {
   /* shift [npt,ept] up by difference */
   for(spt= ept; spt >= npt; spt--)
     *(spt + diff)= *spt;
 } else if(diff < 0) {
   /* shift [npt,ept] down by difference */
   for(spt= npt; spt <= ept; spt++)
     *(spt + diff)= *spt;
   for(spt= ept + diff + 1; spt <= ept; spt++)
     *spt= 0;
 }
 memcpy(cpt + head_len, num_text, num_len);
 *field_end= (cpt - tag_data) + head_len + num_len;
 return(1);
}
...
int Xorriso_refresh_rlsb_tag(struct XorrisO *xorriso, char *head_buffer,
                             int checksum_block, off_t iso_lba, int flag)
{
 int ret, field_end;
 char md5_bin[16], *tag_pt;
 void *ctx= NULL;

 tag_pt= head_buffer + checksum_block * 2048;

 /* Update position and size fields */
 Xorriso__set_iso_check_tag_off_t(tag_pt, " pos=",
                                  (off_t) checksum_block, &field_end, 0);
 Xorriso__set_iso_check_tag_off_t(tag_pt, " range_start=",
                                  (off_t) 0, &field_end, 0);
 Xorriso__set_iso_check_tag_off_t(tag_pt, " range_size=",
                                  (off_t) checksum_block, &field_end, 0);
 Xorriso__set_iso_check_tag_off_t(tag_pt, " session_start=",
                                  iso_lba, &field_end, 0);
                            ...
                            int flag)
{
 ...
 if(flag & 16) {
   ...
   if(ret > 0) {
     ...
     ret= Xorriso_refresh_rlsb_tag(xorriso, head_buffer, checksum_block,
                                   iso_lba, 0);
     if(ret <= 0)
       return(0);
   }
