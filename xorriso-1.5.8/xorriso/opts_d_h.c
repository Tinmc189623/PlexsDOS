...
bad_set_projid:;
     ...
     } else if(strcmp(cpt, "show_time") == 0) {
       if(i + 1 >= end_idx)
         goto not_enough_exec_arguments;
       i++;
       if(strlen(argv[i]) < 1 || strchr("acm", argv[i][0]) == NULL) {
         sprintf(xorriso->info_text,
                 "show_time: time type does not begin by 'a', 'c', or 'm'");
         goto sorry_ex;
       }
       Findjob_set_action_target(job, 66, argv[i], 0);
     } else {
...
"  -volid_for fs_types volume_id",
"                 Specifies the volume ID text for the given list of comma",
"                 separated filesystem types. Recognized types are:",
"                    ecma119 , joliet , iso_9660_1999 , hfsplus",
"  -volset_id name",
...
"                   get_projid, set_projid, get_projid_minmax, show_time,",
"                   find",
...
"  -perform_opc \"on\"|\"off\"|\"as_growisofs\"",
"                 Explicitly ask drive for Optimum Power Calibration.",
...
"@ENDE_OF_HELPTEXT_(HOPEFULLY_UNIQUELY_SILLY_TEXT)@"
 ...
 break;
   /* gcc-15 complained about up to 63519 being sprintf'ed here:
        sprintf(xorriso->result_line,"%s\n",tpt);
      But 0 errors are reported with
        valgrind --leak-check=full xorriso/xorriso -help
      Using snprintf(...,1024,...) does not help.
      The whole helptext of xorriso 1.5.7 has only 44175 bytes.
      Whatever, strcpy and strcat are now used to work around.
   */
   strcpy(xorriso->result_line, tpt);
   strcat(xorriso->result_line, "\n");
   Xorriso_result(xorriso,0);
