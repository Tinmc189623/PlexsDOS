					...
					struct burn_session *s, int tnum)
{
	...
	if (nwa > d->nwa)
		d->nwa = nwa;
	/* ts A70214 : possibly adjust already expanded size of track */
	...
	if (o->perform_opc) {
		fprintf(stderr, "libburn_DEBUG: Libburn_pioneer_dvr_216d_with_opC : OPC was already performed due to API setting\n");
	} else if (d->num_opc_tables <= 0 && !o->simulate) {
				...
				struct burn_session *s, int is_last_session)
{
	...
	if (d->current_profile == 0x11 || d->current_profile == 0x14 ||
	    d->current_profile == 0x15) {
		if (((o->perform_opc == 1 && d->num_opc_tables <= 0) ||
	             o->perform_opc == 2) && !o->simulate)
			d->perform_opc(d);
	} else {
		if (o->perform_opc == 2)
			d->perform_opc(d);
