...
/* libdax_audioxtr 
   ...
   Copyright (C) 2006,2025 Thomas Schmitt <scdbackup@gmx.net>,
...
/* @param flag bit0= enable return -3 in some cases which normally return 0
               bit1= read parameters of a recognized suitable format and
                     return success even if normally the return value would 
                     be 0 or -3 because of bad format variant.
   @return >0 success
            0 unrecognized format
           -1 severe error
           -2 path not found
           -3 with flag bit0: 
              recognized format with unsuitable parameters (e.g compression)
*/
int libdax_audioxtr_new(struct libdax_audioxtr **xtr, char *path, int flag)
{
 ...
 o->audio_format= 1;
 o->audio_format_valid= 0;

 o->au_data_location= 0;
 o->au_data_size= 0xffffffff;

 ret= libdax_audioxtr_open(o, (flag & 1) << 1);
 if(ret<=0) {
   if(ret == -3) {
     ret= (flag & 1) ? -3 : 0;
   } else {
...
/* @param flag bit0= do not complain about unrecognizable format
               bit1= read parameters of a recognized suitable format and
                     return 2 even if normally the return value would 
                     be 0 or -3 because of bad format variant.
   @return  0= unidentified format
           -1= cannot open
           -3= unsuitable non-raw format
            1= suitable audio encoding
               (but possibly unsuitable parameters)
*/
static int libdax_audioxtr_open(struct libdax_audioxtr *o, int flag)
{
 ...
 if(ret<=0) {
   if(ret == 0) {
     if(!(flag & 1)) {
       sprintf(msg,"Audio source file has unrecognizable format : %s",o->path);
...
/* @return  0= unidentified format
           -1= unsuitable or damaged RIFF format
           -2= unsuitable WAVE parameters
            1= .wav with suitable audio encoding
               (but possibly unsuitable parameters)
*/
 ...
 while(old_pos < riff_end) {
   ...
   if(strncmp(buf,"fmt ", 4) == 0) {
     ...
     if(ret < 16)
       return(-1);
     old_pos+= 16;
     o->audio_format= (((unsigned char *) buf)[1] << 8) +
                      ((unsigned char *) buf)[0];
     o->audio_format_valid= 1;
     o->msb_first= 0;
     ...
     sprintf(o->fmt_info,
             ".wav , audio_format=0x%x , num_channels=%d , sample_rate=%d , bits_per_sample=%d",
             o->audio_format, o->num_channels, o->sample_rate,
             o->bits_per_sample);
     fmt_seen= 1;
...
/* @return  0= unidentified format
           -1= unsuitable or damaged Sun Audio
            1= .au with suitable audio encoding
               (but possibly unsuitable parameters)
*/
static int libdax_audioxtr_identify_au(struct libdax_audioxtr *o, int flag)
{
 ...
 if(o->bits_per_sample > 0)
   return(1); /* Audio format must be linear PCM */
 return(-1);
}


/* @return  0= unidentified format
           -1= unsuitable or damaged non-raw format
           -2= unsuitable WAVE parameters
            1=  suitable audio encoding
               (but possibly unsuitable parameters)
*/
...
int libdax_audioxtr_get_wavfmt(struct libdax_audioxtr *xtr,
                               unsigned int *audio_format)
{
 if(!xtr->audio_format_valid) {
   *audio_format= 0;
   return(0);
 }
 *audio_format= xtr->audio_format;
 return(1);
}
