...
/* Copyright (c) 2004 - 2006 Derek Foreman, Ben Jansens
   Copyright (c) 2006 - 2026 Thomas Schmitt <scdbackup@gmx.net>
...
/** NOTE: This call is traditionally without any effect on the operations of
          libburn. Due to this 20 year old tradition it will stay so.
          Use  burn_write_opts_set_perform_opc_v2()  if real Optimum Power
          Calibration is desired.
 
    Wrong traditional description:
    Sets whether to use opc or not with the write_opts struct
    @param opts The write opts to change
    @param opc If non-zero, optimum power calibration will be performed at
               start of burn
...
/** Control whether and when to use Optimum Power Calibration (OPC).
    @param opts The write opts to change
    @param opc  0= Do not perform OPC (default)
                1= Perform OPC like growisofs does:
                   Only with DVD-R[W] and only if the drive does not indicate
                   the presence of internal OPC table entries.
                2= Perform OPC unconditionally on all media types.
    @return     1=opc value was accepted, 0=opc value was rejected
    @since 1.5.8
*/
...
/** Open an audio file, check whether suitable, create extractor object.
    ...
    @param flag Bitfield for control purposes
                bit0= enable return -3 in some cases which normally return 0
                      @since 1.5.8
                bit1= read parameters of a recognized suitable format and
                      return 2 if normally the return value would be 0 or -3
                      because of a bad format variant.
                      @since 1.5.8
    @return  1 success
             2 success, but only because of bit1
             0 unrecognized format, possibly raw
            -1 severe error
            -2 path not found
            -3 with flag bit0:
...
/** Obtain the audio_format code of a possible WAVE file.
    @param xtr Opaque handle to extractor
    @param audio_format returns WAVE Audio Format.
                        1=Linear quantization, others are unusable
    @return 1= returned audio_format is valid
            0= no audio_format available, e.g. because not a WAVE file
    @since 1.5.8
*/
