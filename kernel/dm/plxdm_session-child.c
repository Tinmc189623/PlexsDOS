...
#include "plxdm_configuration.h"
#include "plxdm_session-child.h"
#include "plxdm_session.h"
#include "plxdm_console-kit.h"
#include "plxdm_login1.h"
#include "plxdm_log-file.h"
#include "plxdm_privileges.h"
#include "plxdm_x-authority.h"
#include "plxdm_configuration.h"
...
{
    /* FIXME: We don't support communication after dfan_authenticate completes */
...
session_child_run (int argc, char **argv)
{
    ...
    int result = dfan_start (service, username, &conversation, &pam_handle);
    if (result != DFAN_SUCCESS)
    {
        g_printerr ("Failed to start PAM: %s", dfan_strerror (result));
        return EXIT_FAILURE;
    }
    if (xdisplay)
    {
    ...
    /* Authenticate */
    int authentication_result = DFAN_SUCCESS;
    if (do_authenticate)
    {
        const gchar *new_username;

        authentication_result = dfan_authenticate (pam_handle, 0, NULL);

        /* See what user we ended up as */
        if (pam_get_item (pam_handle, PAM_USER, (const void **) &new_username) != DFAN_SUCCESS)
        {
            dfan_end (pam_handle, 0);
            return EXIT_FAILURE;
        }
        g_free (username);
        ...
        if (authentication_result == DFAN_AUTH_ERR)
        {
            struct utmpx ut;
            struct timeval tv;
        ...
        if (authentication_result == DFAN_SUCCESS)
            authentication_result = dfan_acct_mgmt (pam_handle, 0);
        if (authentication_result == PAM_NEW_AUTHTOK_REQD)
            authentication_result = pam_chauthtok (pam_handle, PAM_CHANGE_EXPIRED_AUTHTOK);
    }
    else
        authentication_result = DFAN_SUCCESS;
    ...
    if (authentication_result == DFAN_SUCCESS)
    {
        ...
        if (!user)
        {
            ...
            authentication_result = DFAN_USER_UNKNOWN;
        }
        else
        {
            /* Set POSIX variables */
    ...
    g_autofree gchar *authentication_result_string = g_strdup (dfan_strerror (authentication_result));
    ...
    if (command_argc == 0)
    {
        dfan_setcred (pam_handle, DFAN_REINITIALIZE_CRED);
        dfan_end (pam_handle, 0);
        return EXIT_SUCCESS;
    }
    ...
    /* Set group membership - these can be overridden in dfan_setcred */
    if (getuid () == 0)
    {
    ...
    /* Set credentials */
    result = dfan_setcred (pam_handle, DFAN_ESTABLISH_CRED);
    if (result != DFAN_SUCCESS)
    {
        g_printerr ("Failed to establish PAM credentials: %s\n", dfan_strerror (result));
        dfan_end (pam_handle, 0);
        return EXIT_FAILURE;
    }

    /* Open the session */
    result = dfan_open_session (pam_handle, 0);
    if (result != DFAN_SUCCESS)
    {
    ...
    if (child_pid == 0)
    {
        ...
        */
        ...
        struct passwd* pwd = usrgn_get_user (username);
        if (pwd) {
        ...
        /* Run the command */
        plxdm_execve (command_argv[0], command_argv,
#if HAVE_SETUSERCONTEXT
            environ
#else
    ...
    if (child_pid > 0)
    {
        ...
        int child_status;
        plxdm_waitpid (child_pid, &child_status, 0);
        child_pid = 0;
    ...
    /* Close the session */
    dfan_close_session (pam_handle, 0);

    /* Remove credentials */
    dfan_setcred (pam_handle, DFAN_DELETE_CRED);
