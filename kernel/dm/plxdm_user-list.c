...
common_user_list_get_user_by_name (CommonUserList *user_list, const gchar *username)
{
    ...
    /* PlexsDOS: 所有用户通过 AccountsService / usrgn 管理, 无 passwd 回退 */
    return NULL;
}
...
common_user_get_gid (CommonUser *user)
{
    ...
    {
        /* PlexsDOS: 使用 usrgn 组 ID */
        const struct usrgn_user *uentry = usrgn_get_user_by_uid (priv->uid);
        if (uentry != NULL)
            priv->gid = uentry->gid;
    }
    return priv->gid;
...
PLXDM_DEFINE_TYPE_GET_TYPE (CommonUserList, common_user_list, G_TYPE_OBJECT)

PLXDM_DEFINE_TYPE_GET_TYPE (CommonUser, common_user, G_TYPE_OBJECT)
