...
{
    const struct usrgn_user *entry = usrgn_get_user_by_uid (getuid ());
    if (entry != NULL)
        return accounts_get_user_by_name (entry->name);
    else
        return NULL;
}
...
PLXDM_DEFINE_TYPE_GET_TYPE (User, user, G_TYPE_OBJECT)
