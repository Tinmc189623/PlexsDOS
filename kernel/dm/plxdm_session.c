...
#include "plxdm_guest-account.h"
#include "plxdm_shared-data-manager.h"
#include "plxdm_greeter-socket.h"
...
void
session_set_auth_service (Session *session, const gchar *auth_service)
{
    SessionPrivate *priv = session_get_instance_private (session);
    g_return_if_fail (session != NULL);
...
session_real_start (Session *session)
{
    ...
    /* Send configuration */
    write_string (session, priv->auth_service);
    ...
    l_debug (session, "Started with service '%s', username '%s'", priv->auth_service, priv->username);
...
{
    g_return_if_fail (session != NULL);
    g_return_if_fail (error != DFAN_SUCCESS);
...
session_get_is_authenticated (Session *session)
{
    SessionPrivate *priv = session_get_instance_private (session);
    g_return_val_if_fail (session != NULL, FALSE);
    return priv->authentication_complete && priv->authentication_result == DFAN_SUCCESS;
}
...
PLXDM_DEFINE_TYPE_GET_TYPE (Session, session, G_TYPE_OBJECT)
