...
void
greeter_set_auth_services (Greeter *greeter, const gchar *auth_service, const gchar *autologin_auth_service)
{
    GreeterPrivate *priv = greeter_get_instance_private (greeter);
    g_return_if_fail (greeter != NULL);
    g_free (priv->auth_service);
    priv->auth_service = g_strdup (auth_service);
    g_free (priv->autologin_auth_service);
    priv->autologin_auth_service = g_strdup (autologin_auth_service);
}
...
handle_authenticate (Greeter *greeter, guint32 sequence_number, const gchar *username)
{
    ...
    if (!priv->authentication_session)
    {
        send_end_authentication (greeter, sequence_number, "", DFAN_USER_UNKNOWN);
        return;
    }
    ...
    {
        service = priv->autologin_auth_service;
        is_interactive = FALSE;
    }
    else
    {
        service = priv->auth_service;
        is_interactive = TRUE;
    }

    /* Run the session process */
    session_set_auth_service (priv->authentication_session, service);
...
handle_authenticate_as_guest (Greeter *greeter, guint32 sequence_number)
{
    ...
    send_end_authentication (greeter, sequence_number, "", DFAN_SUCCESS);
}
...
handle_authenticate_remote (Greeter *greeter, const gchar *session_name, const gchar *username, guint32 sequence_number)
{
    ...
    if (!service)
    {
        send_end_authentication (greeter, sequence_number, "", DFAN_SYSTEM_ERR);
        return;
    }
...
greeter_finalize (GObject *object)
{
    ...
    g_clear_pointer (&priv->auth_service, g_free);
    g_clear_pointer (&priv->autologin_auth_service, g_free);
...
PLXDM_DEFINE_TYPE_GET_TYPE (Greeter, greeter, G_TYPE_OBJECT)
