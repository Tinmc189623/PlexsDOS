...
#include "plxdm_display-manager.h"
#include "plxdm_display-manager-service.h"
#include "plxdm_xdmcp-server.h"
#include "plxdm_vnc-server.h"
#include "plxdm_seat-xdmcp-session.h"
#include "plxdm_seat-xvnc.h"
#include "plxdm_x-server.h"
#include "plxdm_process.h"
#include "plxdm_session-child.h"
#include "plxdm_shared-data-manager.h"
#include "plxdm_user-list.h"
...
/*
 * plxdm_lightdm_start — 内核可调用的 DM 入口
 *
 * 以默认参数调用 main(), 供 kernel_main 或 Shell 使用。
 */
int plxdm_lightdm_start(void)
{
    char *argv[] = { "lightdm", NULL };
    return main(1, argv);
}
